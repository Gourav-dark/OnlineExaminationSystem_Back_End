        ...
        public async Task<ActionResult> Login(Login loginuser)
        {
            ...
            if (user != null)
            {
                if (CommanFunctions.VerifyPassword(loginuser.Password, user.Password))
                {
                    ...
                    string jwt = JwtTokenCreate(user.Email,role.RoleName);
                    return Ok(new { Massage="Login Done",Token=jwt });
                }
                return BadRequest("Incorrect Password");
            }
            return NotFound("Email Id is Not Found");
        }
        //JWT with Role Base Token Generate--------------------
        private string JwtTokenCreate(string email,string role)
        {
